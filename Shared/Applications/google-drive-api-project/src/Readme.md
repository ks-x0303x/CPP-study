...
設定ファイル（`credentials.json` / `token.json`）は、実行環境で用意されている **設定ディレクトリを優先**して参照します。設定ディレクトリに必要ファイルが無い場合は、この README と同じディレクトリ（各 `.sh` / `.py` がある場所）を参照します。
...
2. 取得した `credentials.json` を以下いずれかに配置します。

   - 設定ディレクトリ（実行環境で設定ファイルを置く場所）
   - この README と同じディレクトリ（`auth_quickstart.sh` がある場所）
...
- `auth_quickstart.sh` は依存をインストールし、`quickstart.py` を実行して `token.json` を生成/更新します。
...
`token.json` がこのディレクトリに無い場合は、設定ディレクトリ側にある `token.json` を削除してから再実行してください。
...
Drive 上のファイル名はワイルドカード（`* ? []`）で指定できます。複数一致した場合は **すべてダウンロード**します（このとき `output_path` はディレクトリ指定にしてください）。

```bash
./download_file.sh 'test.*' ./
./download_file.sh 'SomeFolder/*.txt' ./downloads
```

注意: `*` はシェルが展開することがあるので、上記のように **クォート**するのがおすすめです。
...
ローカルのファイルパスもワイルドカード（`* ? []`）で複数指定できます。複数一致した場合は **すべてアップロード**します。

```bash
./upload_file.sh ./test*.txt ./
./upload_file.sh ./local*.txt 'BackupFolder/SubFolder'
```

注意: `*` はシェルが展開することがあるため、パターンとして渡したい場合は `./upload_file.sh './local*.txt' 'BackupFolder/SubFolder'` のように **クォート**してください。
...
`quickstart.py` は認可 URL を表示するので **ブラウザで URL を開いて**ください。
